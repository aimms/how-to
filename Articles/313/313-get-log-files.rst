...
Use the ``LoggerConfig.xml`` file which can be downloaded :download:`here <model/LoggerConfig.xml>` and copy this in the cache of the published application, which can be found at ``~\AppData\Local\AIMMS\PRO\<pro name>\<App Name>\<UUID>\ ``.
