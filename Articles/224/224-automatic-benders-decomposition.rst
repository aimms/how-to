...
The Benders’ decomposition module in AIMMS is implemented as an openalgorithm using GMP functionality. It offers many features. Besides theclassic approach the module contains a modern variant that solves onemaster (MIP) problem using solver callbacks. (See the great blog post byPaul Rubin about `Benders Decomposition Then and Now <https://orinanobworld.blogspot.com/2011/10/benders-decomposition-then-and-now.html>`_.)
To use Benders' decomposition first the system module``GMPBendersDecomposition`` has to be installed. To solve a MIP problemusing the classic approach you can then simply use:
.. code-block:: aimms    :linenos:
